...
def main(args):
    ...
    for i in tqdm(range(len(final_score_list))):
        result = pd.DataFrame(final_score_list[i][1],columns = ['th_idx','cour_cd','score'])
        result['std_id'] = final_score_list[i][0]
        result_f.append(result)
        
    result_ff = pd.concat(result_f, ignore_index=True)
    result_ff = result_ff[result_ff['score']>0]
    result_ff.to_csv("./rgcn_course_rec_result.csv", encoding = 'utf8', sep='\t')
    return result_ff
